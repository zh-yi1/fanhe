#!/usr/bin/env python3
"""Generate mode-page GPU bins from Output/bin/ui/new_ui/*.png.

Icons:
  new_order_0/1   Delay (预约)
  new_chicken_0/1 Chicken
  new_pasta_0/1   Pasta
  new_warm_0/1    Warm
  new_chage       Selected row background
  new_left        Chevron arrow

Run: python tools/gen_new_mode_icons.py
...
OUT_H = ROOT / "functions" / "new_mode_res.h"
...
def macro_prefix(stem: str) -> str:
    return "NEW_MODE_" + stem.upper().replace(".", "_")
...
def main() -> None:
    ...
    lines = [
        ...
        "",
        f"#define NEW_MODE_ICON_RAM_SIZE          {ram_max}",
        "",
    ]
    ...
    if "new_chage" in sizes:
        ...
        lines.append(f"#define NEW_MODE_SEL_BG_W                 {w}")
        lines.append(f"#define NEW_MODE_SEL_BG_H                 {h}")
        lines.append("")

    if "new_order_0" in sizes:
        w, h, _ = sizes["new_order_0"]
        lines.append(f"#define NEW_MODE_ICON_W                   {w}")
        lines.append(f"#define NEW_MODE_ICON_H                   {h}")
        lines.append("")

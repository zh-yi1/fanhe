#!/usr/bin/env python3
"""Scale font_asc.bin to target pixel height (FONT v1)."""
...
def decode_glyph_space(blob: bytes, target_h: int) -> tuple[int, int, Image.Image] | None:
    if len(blob) < 4 or blob[0] != 0x09:
        return None
    width = struct.unpack_from("<H", blob, 0)[0]
    if width == 0:
        width = 4
    img = Image.new("L", (width, max(1, target_h // 3)), 0)
    return width, img.height, img


def decode_glyph_pil(ch: str, width_hint: int, target_h: int) -> Image.Image:
    try:
        font = ImageFont.truetype("arial.ttf", target_h)
    except OSError:
        try:
            font = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", target_h)
        except OSError:
            font = ImageFont.load_default()
    pad = 2
    canvas = Image.new("L", (max(width_hint, 8) + pad * 2, target_h + pad * 2), 0)
    draw = ImageDraw.Draw(canvas)
    draw.text((pad, pad), ch, fill=255, font=font)
    bbox = canvas.getbbox()
    if bbox is None:
        return Image.new("L", (4, target_h), 0)
...
def build_scaled_font(src: bytes, target_h: int) -> bytes:
    ...
    for i in range(GLYPH_COUNT):
        ...
        if decoded is None and i == 0:
            decoded = decode_glyph_space(blob, target_h)
        if decoded is not None:
            _width, _height, img = decoded
            img = scale_image(img, scale)
        else:
            hint = blob[1] if len(blob) > 1 else 8
            img = decode_glyph_pil(ch, max(4, int(hint * scale)), target_h)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Scale font_asc.bin to N px height")
    parser.add_argument("height", type=int, nargs="?", default=12, help="target height in px")
    args = parser.parse_args()
    if args.height < 8 or args.height > 32:
        raise SystemExit("height should be 8..32")

    if not SRC_FONT.exists():
        raise SystemExit(f"missing source font: {SRC_FONT}")

    out_font = SRC_FONT.parent / f"font_asc_{args.height}.bin"
    src = SRC_FONT.read_bytes()
    out = build_scaled_font(src, args.height)
    out_font.write_bytes(out)
    print(
        f"wrote {out_font.name}: height {src[14]} -> {args.height}, "
        f"size {len(src)} -> {len(out)} (0x{len(out):x})"
    )

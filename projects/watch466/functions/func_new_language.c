...
static void new_lang_sel_prev(f_new_lang_t *f)
{
    u8 prev;
    ...
    prev = f->sel;
    f->sel = (f->sel == 0) ? (u8)(NEW_LANG_ITEM_CNT - 1) : (u8)(f->sel - 1);
#if ELUNCHBOX_PANEL_EN
    new_lang_sel_apply_delta(f, prev, f->sel);
#else
    f->display_pending = true;
...
/* 模式键：跳转到模式选择页 */
static void new_lang_mode_key(void)
{
    if (sys_cb.flag_swithing) {
        return;
    }
...
static void func_new_language_process(void)
{
    ...
    if (!f->key_ready) {
        if (f->display_pending) {
            /* 首帧: 先 TE block 阻止新帧，等 GPU（快速），再绑图标 */
            {

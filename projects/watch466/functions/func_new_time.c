 ...
 *   白底卡片：时/分调节 + AM/PM + NO/YES
 *   确认键逐步切换焦点；+/- 调节时/分；电源键回退
 */
#define NEW_TIME_STATUS_Y                  20
...
#define NEW_TIME_HOUR_X                    98
#define NEW_TIME_MIN_X                     218
#define NEW_TIME_BOX_Y                     130
#define NEW_TIME_COLON_X                   158
#define NEW_TIME_ARROW_UP_Y                88
#define NEW_TIME_ARROW_DOWN_Y              172

#define NEW_TIME_AMPM_X                    268
#define NEW_TIME_AM_Y                      118
#define NEW_TIME_PM_Y                      148

#define NEW_TIME_BTN_NO_X                  118
#define NEW_TIME_BTN_YES_X                 228
#define NEW_TIME_BTN_Y                     198

#define NEW_TIME_DIGIT_Y                   (NEW_TIME_BOX_Y - 6)
#define NEW_TIME_H10_X                     (NEW_TIME_HOUR_X - 16)
#define NEW_TIME_H1_X                      (NEW_TIME_HOUR_X + 4)
#define NEW_TIME_M10_X                     (NEW_TIME_MIN_X - 16)
#define NEW_TIME_M1_X                      (NEW_TIME_MIN_X + 4)
#define NEW_TIME_SUFFIX_H_X                (NEW_TIME_HOUR_X + 22)
#define NEW_TIME_SUFFIX_MIN_X              (NEW_TIME_MIN_X + 22)
#define NEW_TIME_SUFFIX_Y                  (NEW_TIME_BOX_Y + 18)
...
static bool new_time_load_hour_digit(u8 slot, u8 digit, compo_picturebox_t *pic, s16 x, s16 y)
{
    if (digit > 9 || slot >= 2) {
        return false;
    }
    if (!new_time_gpu_ram_bind(home_ui_digit_ram[slot], HOME_DIGIT_RAM_MAX_SIZE,
                               ...
                               tbl_new_time_hour_digit_len[digit], pic,
                               tbl_new_time_hour_digit_w[digit],
                               tbl_new_time_hour_digit_h[digit], x, y)) {
        return false;
    }
    return true;
}

static bool new_time_load_min_digit(u8 slot, u8 digit, compo_picturebox_t *pic, s16 x, s16 y)
{
    if (digit > 9 || slot >= NEW_TIME_DIGIT_SLOTS) {
        return false;
    }
    if (!new_time_gpu_ram_bind(home_ui_digit_ram[slot], HOME_DIGIT_RAM_MAX_SIZE,
                               tbl_new_time_min_digit_addr[digit],
                               tbl_new_time_min_digit_len[digit], pic,
                               tbl_new_time_min_digit_w[digit],
                               tbl_new_time_min_digit_h[digit], x, y)) {
        return false;
    }
    return true;
}
...
static void new_time_title_show(compo_textbox_t *txt)
{
    ...
    compo_textbox_set_location(txt, NEW_TIME_TITLE_X, NEW_TIME_TITLE_Y,
...
static void new_time_shapes_apply(f_new_time_t *f)
{
    ...
    if (f->shape_hour_bg != NULL) {
        compo_shape_set_color(f->shape_hour_bg,
                              hour_sel ? NEW_TIME_COLOR_SEL : NEW_TIME_COLOR_BOX_NOR);
        compo_shape_set_visible(f->shape_hour_bg, true);
    }
...
static void new_time_digits_apply_hour(f_new_time_t *f)
{
    ...
    printf("nt_dig_h: %u%u sel=%u\n", h10, h1, hour_sel);

#if ELUNCHBOX_PANEL_EN
    if (hour_sel) {
        (void)new_time_load_hour_digit(0, h10, f->pic_h10, NEW_TIME_H10_X, NEW_TIME_DIGIT_Y);
        (void)new_time_load_hour_digit(1, h1, f->pic_h1, NEW_TIME_H1_X, NEW_TIME_DIGIT_Y);
    } else {
        (void)new_time_load_min_digit(0, h10, f->pic_h10, NEW_TIME_H10_X + 4, NEW_TIME_DIGIT_Y + 8);
        (void)new_time_load_min_digit(1, h1, f->pic_h1, NEW_TIME_H1_X + 4, NEW_TIME_DIGIT_Y + 8);
    }
    if (f->pic_colon != NULL) {
        (void)new_time_gpu_ram_bind(home_ui_colon_ram, HOME_COLON_RAM_SIZE,
                                    UI_BUF_NEW_UI_NEW_COLONM_BIN, UI_LEN_NEW_UI_NEW_COLONM_BIN,
                                    f->pic_colon, HOME_TOP_TIME_COLONM_W, HOME_TOP_TIME_COLONM_H,
                                    NEW_TIME_COLON_X, NEW_TIME_DIGIT_Y + 8);
    }
#endif
}
...
static void new_time_digits_apply_min(f_new_time_t *f)
{
    ...
    printf("nt_dig_m: %u%u sel=%u\n", m10, m1, min_sel);

#if ELUNCHBOX_PANEL_EN
    if (min_sel) {
        (void)new_time_load_hour_digit(2, m10, f->pic_m10, NEW_TIME_M10_X, NEW_TIME_DIGIT_Y);
        (void)new_time_load_hour_digit(3, m1, f->pic_m1, NEW_TIME_M1_X, NEW_TIME_DIGIT_Y);
    } else {
        (void)new_time_load_min_digit(2, m10, f->pic_m10, NEW_TIME_M10_X, NEW_TIME_DIGIT_Y + 8);
        (void)new_time_load_min_digit(3, m1, f->pic_m1, NEW_TIME_M1_X, NEW_TIME_DIGIT_Y + 8);
    }
#endif
}
...
compo_form_t *func_new_time_form_create(void)
{
    ...
    (void)new_time_txt_create(frm, COMPO_ID_TXT_TITLE, 8,
                              NEW_TIME_TITLE_X, NEW_TIME_TITLE_Y,
                              NEW_TIME_TITLE_W, NEW_TIME_TITLE_H,
                              NEW_TIME_COLOR_TITLE, false);

    bat_x = (s16)(GUI_SCREEN_WIDTH - NEW_TIME_STATUS_RIGHT_MARGIN - NEW_HOME_BAT_W / 2);
    bt_x = (s16)(bat_x - NEW_HOME_BAT_W / 2 - NEW_TIME_STATUS_GAP - NEW_HOME_BT_W / 2);

    pic = new_time_pic_create_hidden(frm, COMPO_ID_PIC_BT);
    compo_picturebox_set_pos(pic, bt_x, NEW_TIME_STATUS_Y);
    compo_picturebox_set_size(pic, NEW_HOME_BT_W, NEW_HOME_BT_H);
    pic = new_time_pic_create_hidden(frm, COMPO_ID_PIC_BAT);
    compo_picturebox_set_pos(pic, bat_x, NEW_TIME_STATUS_Y);
    compo_picturebox_set_size(pic, NEW_HOME_BAT_W, NEW_HOME_BAT_H);

    (void)new_time_shape_create(frm, COMPO_ID_SHAPE_HOUR_BG,
                                NEW_TIME_HOUR_X, NEW_TIME_BOX_Y,

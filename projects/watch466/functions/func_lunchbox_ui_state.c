/**
 * @file    func_lunchbox_ui_state.c
 * @brief   设备状态结构体实现 — 见 func_lunchbox_ui_state.h
 */
#include "include.h"
...
{
    if (lb_time.synced) {
        u32 unix_time = lb_get_unix_time();
        return time_to_tm(unix_time - LB_RTC_UNIX_OFFSET);
    }
    return rtc_clock_get();
}

//-----------------------------------------------------------------------------
// 模式预设 — 各加热模式的默认参数, 按模式号(DP2)索引; APP 可下发修改
//-----------------------------------------------------------------------------

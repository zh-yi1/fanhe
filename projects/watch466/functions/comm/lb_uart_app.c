...
//-----------------------------------------------------------------------------
// manual_off 深睡唤醒原因 (见 lb_uart_app.h 注释)
// 曾有 lunchbox_wake_probe() 在睡眠循环里偷听判帧, 实测收帧不完整, 已删。
//-----------------------------------------------------------------------------
...
{
    // KEY 优先级更高 (按键唤醒的同时来了帧, 按开机处理)
    if (lb_wake_reason != LB_WAKE_KEY) {
        lb_wake_reason = LB_WAKE_UART;
    }
}

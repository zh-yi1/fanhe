...
static u8 pasta_temp_pic_idx(u8 temp_idx)
{
...
static u8 pasta_preset_time_min(void)
{
    u32 d = lunchbox_mode_get_duration(LB_MODE_PASTA);
...
/* ---- 显示刷新 ---- */
static void heat_pasta_update_display(void)
{
    f_heat_pasta_t *inf = (f_heat_pasta_t *)func_cb.f_cb;
    char buf[16];
...
/* ---- 按键处理 ---- */
static void func_heat_pasta_handle_keys(void)
{
    f_heat_pasta_t *inf = (f_heat_pasta_t *)func_cb.f_cb;
    ...
    while (func_key_get_event(&evt)) {
        ...
        case FUNC_KEY_CONFIRM:
            /* 下发意面模式加热: 温度取预设表(不可改), 时长用页面选的 */
#if FUNC_LUNCHBOX_UART_EN
            {
                u8 t_idx = lunchbox_mode_get_temp(LB_MODE_PASTA);
                lb_heat_cmd_start(LB_MODE_PASTA, t_idx, inf->time_min);
                printf("pasta: start %uF %umin\n",
...
/* ---- 每帧处理 ---- */
static void func_heat_pasta_process(void)
{
    f_heat_pasta_t *inf = (f_heat_pasta_t *)func_cb.f_cb;
...
static void func_heat_pasta_message(size_msg_t msg)
{
    func_message(msg);
}
...
compo_form_t *func_heat_pasta_form_create(void)
{
    ...
    inf->tmp_pic = compo_picturebox_create(frm,
            TMP_SCHEDULE_PICS[pasta_temp_pic_idx(lunchbox_mode_get_temp(LB_MODE_PASTA))]);
...
void func_heat_pasta_enter(void)
{
    ...
    func_cb.f_cb = func_zalloc(sizeof(f_heat_pasta_t));
    func_key_reset();
    func_cb.frm_main = func_heat_pasta_form_create();
    inf = (f_heat_pasta_t *)func_cb.f_cb;
    inf->display_stage = 1;
    inf->time_min = pasta_preset_time_min();   /* 预设时长, APP 可经 0x0a 修改 */
    heat_pasta_update_display();
...
void func_heat_pasta(void)
{
    printf("%s\n", __func__);
    func_heat_pasta_enter();
    while (func_cb.sta == FUNC_NEW_HEAT_PASTA)
    {
        func_heat_pasta_process();
        func_heat_pasta_message(msg_dequeue());
    }
    func_heat_pasta_exit();
}

 ...
 *   }
 */
#ifndef __FUNC_LUNCHBOX_UI_STATE_H
#define __FUNC_LUNCHBOX_UI_STATE_H
...
//-----------------------------------------------------------------------------
// 状态查询快捷接口 (读 lb_ui_state, 供旧调用方/UI 直接用)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// 时间服务 (APP 权威时间优先, 未同步回退本地 RTC)
//-----------------------------------------------------------------------------

/** @brief 记录 APP 下发的权威时间戳 (BLE 业务层调用) */
...
/** @brief 屏幕显示时间: 已同步用权威时间, 否则本地 RTC */
tm_t lb_get_display_tm(void);

//-----------------------------------------------------------------------------
// 模式预设 (温度档位 + 时长; APP 经 0x0a 修改, 模式页读取)
//-----------------------------------------------------------------------------

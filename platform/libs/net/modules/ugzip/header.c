...
int uzlib_parse_zlib_gzip_header(uzlib_uncomp_t *d, int *wbits)
{
    ...
    if (cmf == 0x1f && flg == 0x8b) {
        ...
        if (uzlib_get_byte(d) != 8){ uart_putchar('!');uart_putchar('1');return UZLIB_DATA_ERROR;}

        /* get flag byte */
        flg = uzlib_get_byte(d);

        /* check that reserved bits are zero */
        if (flg & 0xe0) { uart_putchar('!');uart_putchar('2');return UZLIB_DATA_ERROR;}
    ...
    //          return UZLIB_DATA_ERROR;
       ...
       /* check checksum */
       if ((256*cmf + flg) % 31) { uart_putchar('!');uart_putchar('3');return UZLIB_DATA_ERROR;}

       /* check method is deflate */
       if ((cmf & 0x0f) != 8) { uart_putchar('!');uart_putchar('4');return UZLIB_DATA_ERROR;}

       /* check window size is valid */
       if ((cmf >> 4) > 7) { uart_putchar('!');uart_putchar('6');return UZLIB_DATA_ERROR;}

       /* check there is no preset dictionary */
       if (flg & 0x20) { uart_putchar('!');uart_putchar('6');return UZLIB_DATA_ERROR;}

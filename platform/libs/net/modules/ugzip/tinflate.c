...
unsigned char uzlib_get_byte(uzlib_uncomp_t *d)
{
    ...
    if (d->source_read_cb && !d->eof) {
        ...
        int val = d->source_read_cb((void *)d);
        if (val >= 0) {
...
static int tinf_decode_symbol(uzlib_uncomp_t *d, TINF_TREE *t)
{
   ...
   do {
      ...
      if (++len == TINF_ARRAY_SIZE(t->table)) {
        { uart_putchar('!');uart_putchar('7');return UZLIB_DATA_ERROR;}
      }
   ...
   if (sum < 0 || sum >= TINF_ARRAY_SIZE(t->trans)) {
     { uart_putchar('!');uart_putchar('8');return UZLIB_DATA_ERROR;}
   }
   #endif
...
static int tinf_decode_trees(uzlib_uncomp_t *d, TINF_TREE *lt, TINF_TREE *dt)
{
   ...
   for (num = 0; num < hlimit; )
   {
      ...
      case 16:
         ...
         if (num == 0) { uart_putchar('!');uart_putchar('9');return UZLIB_DATA_ERROR;}
      ...
      if (num + length > hlimit) { uart_putchar('!');uart_putchar('a');return UZLIB_DATA_ERROR;}
      for (; length; --length)
      {
   ...
   if (lengths[256] == 0) {
      { uart_putchar('!');uart_putchar('b');return UZLIB_DATA_ERROR;}
   }
   #endif
...
static int tinf_inflate_block_data(uzlib_uncomp_t *d, TINF_TREE *lt, TINF_TREE *dt)
{
    if (d->curlen == 0) {
        ...
        if (d->eof) {
            { uart_putchar('!');uart_putchar('c');return UZLIB_DATA_ERROR;}
        }
        ...
        sym -= 257;
        if (sym >= 29) {
            { uart_putchar('!');uart_putchar('d');return UZLIB_DATA_ERROR;}
        }
        ...
        if (dist >= 30) {
            { uart_putchar('!');uart_putchar('e');return UZLIB_DATA_ERROR;}
        }
        ...
        if (d->dict_ring) {
            ...
            if (offs > (unsigned int)(d->dest - d->dest_start)) {
                printf("offs:%d, 0x%x, 0x%x\n", offs, d->dest, d->dest_start);
                { uart_putchar('!');uart_putchar('f');return UZLIB_DATA_ERROR;}
            }
            d->lzOff = -offs;
        }
    }
...
static int tinf_inflate_uncompressed_block(uzlib_uncomp_t *d)
{
    if (d->curlen == 0) {
        ...
        /* check length */
        if (length != (~invlength & 0x0000ffff)) { uart_putchar('!');uart_putchar('g');return UZLIB_DATA_ERROR;}
...
{
   memset(d, 0, sizeof(uzlib_uncomp_t));
...
next_blk:
        ...
        case 2:
            ...
            break;
        default:
            { uart_putchar('!');uart_putchar('h');return UZLIB_DATA_ERROR;}
        }

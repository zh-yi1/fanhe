...
/**
 * 说明接下来的代码将实现带溢出检测的加法和减法运算，
 * 而不带溢出检测的版本以内联函数的形式在头文件中提供
 **/
#ifndef FIXMATH_NO_OVERFLOW     /**
                                 * 条件编译指令, 用于确保只有在没有定义
                                 * FIXMATH_NO_OVERFLOW 宏时, 下面的代码
                                 * 才会被编译, 这允许开发者根据需要启用
                                 * 或禁用溢出检测
                                 **/
...
/**
 * @brief 定点浮点加法运算(带溢出检测)
 * @param[in] a: 定点数 a
 * @param[in] b: 定点数 b
 * @return fix16_t: 加法运算结果
 **/
fix16_t fix16_add(fix16_t a, fix16_t b)
{
	/*
	 * 需要将参数 a 和 b 转换为无符号整数类型 uint32_t，
	 * 因为带有符号的整数溢出是未定义行为，如提供的链接所述
	...
	**/
	...
	/*
	 * 这段代码检查是否发生了溢出。溢出只会在 a 和 b 符号相同，
	 * 并且 a 和结果 sum 的符号不同时发生。
	 **/
	if (!((_a ^ _b) & 0x80000000) && ((_a ^ sum) & 0x80000000))
		return fix16_overflow;          //fix16_overflow 为 const变量来指示溢出发生

	return sum;                         //运算没有溢出，返回运算结果
}
...
fix16_t fix16_sub(fix16_t a, fix16_t b)
{
	...
	**/
	...
	/*
	 * 这段代码检查是否发生了溢出。溢出只会在 a 和 b 符号不同，
	 * 并且 a 和结果 sum 的符号不同时发生。
	 **/
	if (((_a ^ _b) & 0x80000000) && ((_a ^ diff) & 0x80000000))
		return fix16_overflow;          //fix16_overflow 为 const变量来指示溢出发生

	return diff;                        //运算没有溢出，返回运算结果
}

/**
 * 这段代码定义了饱和算术（Saturating arithmetic）的两个函数，
 * fix16_sadd 和 fix16_ssub，它们分别用于执行带饱和检测的加法
 * 和减法运算。饱和算术是一种在发生算术溢出时不返回错误或异常，
 * 而是返回最大或最小可能值的算术运算方式。
 **/
...
{
	fix16_t result = fix16_add(a, b);       //调用前面定义带溢出检测的fix16_add函数执行加法运算，并储存结果

	/*
	 * 检查加法结果是否为fix16_overflow(溢出)，如果溢出了，
	 * 则根据 a 定点数据的符号返回最大或最小值，如果 a 定
	 * 点数据是非负数，则返回 fix16_maximum(定点库数据精度最大值)
	 * 如果是负数，则返回 fix16_minimum(定点库数据精度最小值)
	 */
...
{
	fix16_t result = fix16_sub(a, b);       //调用前面定义带溢出检测的fix16_sub函数执行减法运算，并储存结果

	/*
	 * 检查减法结果是否为fix16_overflow(溢出)，如果溢出了，
	 * 则根据 a 定点数据的符号返回最大或最小值，如果 a 定
	 * 点数据是非负数，则返回 fix16_maximum(定点库数据精度最大值)
	 * 如果是负数，则返回 fix16_minimum(定点库数据精度最小值)
	 */
...
/**
 * 这段代码是一个用于执行两个16位固定点数相乘的函数 fix16_mul，它使用64位整数进行中间计算。
 * 函数是为 fix16_mul 提供的64位实现，特别适合ARM Cortex M3等处理器。它执行一个32位乘以32位
 * 得到64位结果的乘法操作。中间的32位是最终结果，低16位用于四舍五入，高16位用于溢出检测。
 **/
...
/**
 * @brief 浮点库定点乘法运算
 * @param[in] inArg0: 定点数inArg0
 * @param[in] inArg1: 定点数inArg1
 * @return fix16_t: 乘法运算返回结果
 **/
fix16_t fix16_mul(fix16_t inArg0, fix16_t inArg1)
{
	int64_t product = (int64_t)inArg0 * inArg1;         //计算两个定点数相乘得出的64位结果

	#ifndef FIXMATH_NO_OVERFLOW
//	/*
//	 * 没有定义 FIXMATH_NO_OVERFLOW (选择没有溢出检测) 时
//	 * 则通过右移47位来检查溢出。如果高17位不是全为符号位，
//	 * 则表示发生溢出
//	 */
...
//     */
	...
	#else
//	/*
//	 * 需要四舍五入，则将结果右移16位后，再加上结果低
//	 * 16位的一半（即当结果的第15位为1时，则进1）
//	 */
...
/**
 * 这段代码是一个在16位处理器上可能较快的32位实现的 fix16_mul 函数，
 * 适用于那些编译器不支持 uint64_t（无64位整数类型）的情况。这个函数
 * 使用16位乘以16位得到32位结果的乘法操作来计算两个固定点数的乘积。
 **/
#if defined(FIXMATH_NO_64BIT) && !defined(FIXMATH_OPTIMIZE_8BIT)
fix16_t fix16_mul(fix16_t inArg0, fix16_t inArg1)
{
...
//	/*
//	 * 计算4个乘积 AC整数部分乘积、BD小数部分乘积、
//	 * AD参数inArg0整数部分与参数inArg1小数部分乘积、
//	 * CB参数inArg1整数部分与参数inArg0小数部分乘积、
//	 */
	int32_t AC = A*C;
...
//	/*
//	 * 计算结果高32位部分，由于小数部分乘积永远是小数，即通过
//	 * AC整数部分乘积 + (AD参数inArg0整数部分与参数inArg1小数部分乘积
//	 * + CB参数inArg1整数部分与参数inArg0小数部分乘积) >> 16
//	 * 即可计算结果的高32位值
//	 */
	int32_t product_hi = AC + (AD_CB >> 16);

//	/*
//	 * 计算结果低32位部分，由于小数部分乘积永远是小数，即通过
//	 * BD小数部分乘积 + (AD参数inArg0整数部分与参数inArg1小数部分乘积
//	 * + CB参数inArg1整数部分与参数inArg0小数部分乘积) << 16
//	 * 即可计算结果的低32位值
//	 */
...
#ifdef FIXMATH_NO_ROUNDING
//    //不进行四舍五入，则直接高32位数据直接左移16位+低32位数据右移16位
...
/**
 * 这段代码是一个针对8位微控制器（例如Atmel AVR）优化的
 * fix16_mul 函数实现，使用8位乘以8位得到16位结果的乘法操作
 **/
...
/**
 * 这段代码提供了一个饱和算术（Saturating Arithmetic）
 * 的包装函数 fix16_smul，用于在乘法操作中处理溢出情况。
 */
#ifndef FIXMATH_NO_OVERFLOW
...
/**
 * 32位实现的 fix16_div 函数。对于例如 ARM Cortex M3 来说最快的版本。
 * 反复执行32位除法来减少余数。为此高效，处理器必须具备32位硬件除法。
 **/
#if !defined(FIXMATH_NO_HARD_DIVISION)
//支持硬件32位除法
#ifdef __GNUC__
//使用GNUC编译器则使用下面方法实现计算前导0的数量
// Count leading zeros, using processor-specific instruction if available.
#define clz(x) (__builtin_clzl(x) - (8 * sizeof(long) - 32))
#else
//如果不是使用GNUC编译器
//则通过下面一个通用clz函数实现计算前导0的数量
...
/**
 * 32位实现的 fix16_div 函数的替代版本。在例如 Atmel AVR 上最快。
 * 这个实现手动进行除法操作，因此对于没有硬件除法的处理器是好的选择。
 */
#if defined(FIXMATH_NO_HARD_DIVISION)
//不支持硬件32位除法
fix16_t fix16_div(fix16_t a, fix16_t b)
...
/**
 * 这段代码提供了一个饱和算术（Saturating Arithmetic）
 * 的包装函数 fix16_sdiv，用于在除法操作中处理溢出情况。
 */

...
static u8 avi_jpeg_fifo_buff[AVI_JPEG_FIFO_LEN] AT(.avi_play_fifo.buff);    //AT(.psram_video_play_disp.buf);//放spram解码会慢一些, 需要psram_switch_cache转换一下
static bool avi_is_init;


/*
 *如果启用加密，底层回调这个函数获取KEY
 */

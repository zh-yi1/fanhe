...
int qmc6309_read_block(unsigned char addr, unsigned char *data, unsigned char len)
{
	...
	bsp_hw_i2c_rx_buf(SENSOR_GEO_IICX, i2c_cfg, QMC6309_READ_ADDR(QMC6309_IIC_ADDR), addr, data, len);
...
int qmc6309_write_reg(unsigned char addr, unsigned char data)
{
	...
	bsp_hw_i2c_tx_byte(SENSOR_GEO_IICX, i2c_cfg, QMC6309_WRITE_ADDR(QMC6309_IIC_ADDR), addr, data);
...
//float data[3]
AT(.text.qmc6309)
void qmc6309_axis_convert(float *data, int layout)
{
	float raw[3];
...
AT(.text.qmc6309)
void qmc6309_setrst_auto_mode(short *hw_d)
{
	int ret = QMC6309_FAIL;
...
//short data[3]
// AT(.text.qmc6309)
// int qmc6309_read_mag_raw_xyz(short *data)
// {
// 	int res;
// 	unsigned char mag_data[6];
// //	short hw_d[3] = {0};
...
AT(.com_text.qmc6309)
void qmc6309_20ms_tick(void)
{
	if (p_mag.sta < QMC6309_EN) {
		return;
	}

	//os_mq_sb_post_manual(MANUAL_MSG_GEOSENSOR);
}

AT(.text.qmc6309)
void qmc6309_20ms_process(void)
{
	if (p_mag.sta < QMC6309_EN) {
		return;
	}

	qmc6309_read_mag_xyz(qmc6309_qst.raw.dat);

	convert_magnetic(qmc6309_qst.raw.dat, qmc6309_qst.result.dat, qmc6309_qst.offset.dat, &qmc6309_qst.rr, &qmc6309_qst.accuracy);

	qmc6309_qst.acc.dat[0] = qmc6309_qst.raw.dat[0] * 9.8 / 1000;
	qmc6309_qst.acc.dat[1] = qmc6309_qst.raw.dat[1] * 9.8 / 1000;
	qmc6309_qst.acc.dat[2] = qmc6309_qst.raw.dat[2] * 9.8 / 1000;

	QST_ORI_progress(qmc6309_qst.acc.dat, qmc6309_qst.result.dat, qmc6309_qst.ori.dat, &qmc6309_qst.accuracy);

	qmc6309_qst.org = (int)qmc6309_qst.ori.dat[0];

	static int lori = 0;

	if (lori != qmc6309_qst.org) {
		lori = qmc6309_qst.org;
		printf("org->%d\n", qmc6309_qst.org);
	}
}

AT(.text.qmc6309)
void fit_qmc6309_get_result(int *ori)
{
	if (p_mag.sta < QMC6309_EN) {
		return;
	}
